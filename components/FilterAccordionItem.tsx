...
const FilterAccordionItemComponent = ({
  ...
  return (
    <AccordionItem
      className="w-full mb-3 rounded-[4px] min-h-14 bg-[#0f172a] px-4 border-0"
      value={value}
    >
      <AccordionTrigger
        className="!no-underline text-sm font-semibold flex justify-between items-center"
        style={{ color: '#dc2626' }} 
      >
        {title}
      </AccordionTrigger>
        ...
        {hasSearch && (
          <div className="mb-2">
            <Input
              ...
              className="w-full text-sm focus:!ring-0 focus:!shadow-none boder"
            />
          </div>
        )}
        {filterType === "checkbox" && (
        ...
        {hasClearButton && (
          <div className="mt-2">
            <Button
              variant="ghost"
              onClick={handleClear}
              className="!text-[#dc2626] !bg-transparent !font-medium !text-[13px] uppercase !p-0 !w-auto !h-auto"
            >
              Clear
            </Button>
          </div>
