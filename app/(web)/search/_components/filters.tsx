...
import { CAR_BRAND_OPTIONS, CAR_COLOR_OPTIONS, CAR_CONDITION_OPTIONS, CAR_FUELTYPE_OPTIONS, CAR_MODEL_OPTIONS, CAR_PRICE_RANGE_OPTIONS, CAR_YEAR_RANGE_OPTIONS } from "@/constants/car-options";
...
const [sliderValues, setSliderValues] = useState<number[]>([
  ...
  const handlePriceCustomRange = (values: number[]) => {
    const [min, max] = Array.isArray(values) && values.length >= 2 ? values[0] <= values[1] ? [values[0], values[1]] : [values[1], values[0]] : [0, 0]
    ...
    <div className="space-y-4">
      ...
      >
          ...
          renderCustom={
            <div className="py-1">
              <div className="flex items-center justify-between mb-[5px]">
                <h5 className="font-medium text-sm text-[#dc2626]">Price</h5>
        ...
        title="Years of Manufacture"
          ...
          selectedValues={filters.yearMin && filters.yearMax ? `${filters.yearMin}-${filters.yearMax}` : ""}
          onValuesChange={(values: any) => {
            const [min, max] = values === "" ? [0, 0] : values?.split("-")?.map(Number) || [null, null]
